...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
      ...
      {/* ── Body ── */}
      <div className="flex flex-1 min-h-0 overflow-hidden max-w-screen-2xl w-full mx-auto flex-col md:flex-row">
        {/* Sidebar */}
        <aside
          className={`${
            isThreadPanelOpen ? "flex" : "hidden"
          } md:flex w-full md:w-72 shrink-0 border-b md:border-b-0 md:border-r border-gray-800 flex-col overflow-hidden max-h-[50vh] md:max-h-none`}
        >
        ...
        <main className="flex-1 min-h-0 overflow-y-auto">
          {/* Toolbar */}
          <div className="sticky top-0 z-10 bg-gray-950/95 backdrop-blur border-b border-gray-800 px-3 sm:px-4 py-2 flex flex-wrap items-center gap-2 sm:gap-3">
            <button
              onClick={() => setIsThreadPanelOpen((prev) => !prev)}
              className="md:hidden px-3 py-1.5 rounded-md bg-gray-800 border border-gray-700 text-xs text-gray-200"
            >
              {isThreadPanelOpen ? "Hide threads" : `Threads (${threadsTotal})`}
            </button>

            {/* Type filter tabs */}
            <div className="flex gap-1 bg-gray-800/60 rounded-full px-1 py-1 w-full sm:w-auto overflow-x-auto">
            ...
            <input
              ...
              className="w-full sm:flex-1 sm:min-w-0 sm:max-w-sm rounded-lg px-3 py-1.5 bg-gray-800 border border-gray-700 text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-purple-500"
            />
            ...
            {selectedThread && (
              <div className="flex items-center gap-2 text-sm text-purple-300 bg-purple-900/20 border border-purple-700/40 rounded-full px-3 py-1">
                <span className="truncate max-w-xs">
                  {selectedThread.title || selectedThread.url}
                </span>
                <button
                  onClick={() => {
                    setSelectedThread(null);
            ...
            {/* Count */}
            <span className="text-xs text-gray-500 shrink-0 w-full sm:w-auto sm:ml-auto">
              {mediaTotal}{" "}
              {mediaType === "all"
                ? "items"
                : { image: "images", video: "videos" }[mediaType]}
            </span>
          </div>
